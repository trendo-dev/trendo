...
ui->checkUseObfuscation->setVisible(false);
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Trendo address TEBxEjGXmJxwpXxXAJqeXztdUR4MsZZJsE"));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        ...
        QString address = "<span style='font-family: OpenSans;'>" + rcp.address;
        address.append("</span>");
...
void SendCoinsDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
        ...
        ui->checkBoxMinimumFee->setStyleSheet("color:#273645; font-size: 12px; font: OpenSans ;");
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Trendo address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
