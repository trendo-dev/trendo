...
bool MultisigDialog::createMultisigTransaction(vector<CTxIn> vUserIn, vector<CTxOut> vUserOut, string& feeStringRet, string& errorRet)
{
    try{
        ...
        if(totalIn < totalOut){
            throw runtime_error("Not enough Trendo provided as input to complete transaction (including fee).");
        }
        ...
        if(tx.vout.at(changeIndex).nValue > fee){
           ...
           feeStringRet = strprintf("%d",((double)fee)/COIN).c_str();
            ...
            throw runtime_error("Not enough Trendo provided to cover fee");
        }
...
void MultisigDialog::on_addAddressButton_clicked()
{
    ...
    QIcon icon3;
    icon3.addFile(QStringLiteral(":/icons/white_address-bookk"), QSize(), QIcon::Normal, QIcon::Off);
    ...
    QIcon icon4;
    icon4.addFile(QStringLiteral(":/icons/white_editpaste"), QSize(), QIcon::Normal, QIcon::Off);
    ...
    QIcon icon5;
    icon5.addFile(QStringLiteral(":/icons/white_remove"), QSize(), QIcon::Normal, QIcon::Off);
...
void MultisigDialog::on_addInputButton_clicked()
{
    ...
    QIcon icon;
    icon.addFile(QStringLiteral(":/icons/white_remove"), QSize(), QIcon::Normal, QIcon::Off);
