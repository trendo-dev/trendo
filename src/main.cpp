...
#if defined(NDEBUG)
#error "Trendo cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in utrendo) are considered zero fee (for relaying and mining)
...
//Testnet 
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
			if (nHeight == 0) return 20000000000 * COIN;
        if (nHeight > 0)
            return 1 * COIN;
    }
	...
	int64_t nSubsidy = 0;
		...
		if(nHeight > 0 && nHeight <= 75001) {
        nSubsidy = 0.02 * COIN;
    } else if (nHeight > 75001 && nHeight <= 130001) {
      nSubsidy = 1 * COIN;
    } else if (nHeight > 130001 && nHeight <= 126796631) {
      nSubsidy = 1500 * COIN;
    } else {
      nSubsidy = 0 * COIN;
    }
...
//Testnet 
   ...
   if (nHeight > 130001) {
      ...
      {
          case 1:
              return blockValue * 0.0018;

          case 2:
              return blockValue * 0.009;

          case 3:
              return blockValue * 0.0522;
			  
		  case 4:
              return blockValue * 0.09;
			  
		  case 5:
              return blockValue * 0.747;
      }
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (pindex->nHeight > LIMIT_POS_HEIGHT) {
